...
2. For each item in the backpack:
  - Calculate the cost of the item by multiplying the quantity by the price each.
  - Add the cost to the running total.
3. Display the total value of the backpack.
```

```javascript
let totalValue = 0

for (let i = 0; i < backpackFullOfAmmo.length; i++) {
  totalValue += calculateOrderCost(backpackFullOfAmmo[i])
}
...
<YouTubeEmbed videoid="GnMJ2wTCfYI" height={400} />

The above video reviews loops and arrays and indices, but, most importantly, it reinforces what it means to **think like a developer.**
